...
fun main(args: Array<String>) {
    ...
    val normalizedHourCandles = makeValuesRelative(scaleByMinutes(candles))
            .map{it.multiplyEverythingBy(1000)}

    println("hello")
}

fun getNormalizedHourCandlesFromFile(fileName: String, minutes: Long = 60): List<Candle> {
    val lines = Files.readAllLines(Paths.get(Data::class.java.getResource(fileName).toURI()))

    val candles = lines.asSequence()
            .map { parseCandle(it) }
            .toList()

    return makeValuesRelative(scaleByMinutes(candles, minutes))
            .map { it.multiplyEverythingBy(1000) }
...
fun scaleByMinutes(candles: List<Candle>, minutes: Long = 60L): List<Candle> {
    if (minutes == 1L)
        return candles
